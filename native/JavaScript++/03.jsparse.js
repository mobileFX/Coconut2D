...
var _install = function () 
{
        ...
        Tokenizer.prototype = 
        {
            ...
            get: function () 
            {
                ...
                //} else if ((match = /^"(?!")(?:\\.|[^"])*"|^'(?!')(?:\\.|[^'])*'|^(['"]{3})((?:(?!\1)[\s\S])*)\1|^""|^''/.exec(input))) { // "){
					...
					if(token.value && token.value.indexOf(this.FILE_DELIM)!=-1)
					{
						this.filename = token.value.substr(this.FILE_DELIM.length);
						this.currFileStartLine = this.lineno;
						this.currFileOffset = this.cursor + token.value.length +2;														
						trace("Parsing file: " + this.filename);
					}
	...
	//	  / ____/___  ____ ___  ____  (_) /__  _____/ ____/___  ____  / /____  _  __/ /_
	//	 / /   / __ \/ __ `__ \/ __ \/ / / _ \/ ___/ /   / __ \/ __ \/ __/ _ \| |/_/ __/
	//	/ /___/ /_/ / / / / / / /_/ / / /  __/ /  / /___/ /_/ / / / / /_/  __/>  </ /_  
	//	\____/\____/_/ /_/ /_/ .___/_/_/\___/_/   \____/\____/_/ /_/\__/\___/_/|_|\__/  
	//	                    /_/                                                         
	// ==================================================================================================================================    
    ...
    function Node(t, type) 
    {
        ...
        this.__defineGetter__("nodeType", function() { return GLOBAL.narcissus.jsdefNames[this.type]; });
        this.__defineGetter__("source", function() { return !this.tokenizer ? "" : this.tokenizer.source.slice(this.start, this.end); });
	...
	// ==================================================================================================================================
	//	   _____ __        __                            __ 
	//	  / ___// /_____ _/ /____  ____ ___  ___  ____  / /_
    ...
    function Statement(t, x) 
    {
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        case jsdef.PROPERTY:
            return PropertyDefinition(t, x, true,
        ...
        case jsdef.FOR:
            ...
            else if (n2 && t.match(jsdef.INSIDE)) 
            {
                ...
                if (n2.type == jsdef.VAR || n2.type == jsdef.LET) 
                {
                    if (n2.length != 1) 
                    {
                        throw new SyntaxError("Invalid for..inside left-hand side", t.filename, n2.lineno);
                    }
                    n.iterator = n2;
                    n.varDecl = n2;
                } else {
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    function PropertyDefinition(t, x, requireName, functionForm) 
    {    
    	// Setter: public property foo(v:DataType){}
    	// Getter: public property foo:DataType
    	
    	var f = new Node(t);    	
    	f.type = jsdef.PROPERTY;
        f.params = [];
        f.paramsList = [];
        f.scopeId = scopeId;        	            	
        f.static = t.static;
        ...
        if(t.peek()==jsdef.LEFT_PAREN) 
        {
        	// Setter
        	f.setter = true;
			t.mustMatch(jsdef.LEFT_PAREN);	            	            
			t.mustMatch(jsdef.IDENTIFIER);	            
            f.propVar = t.token().value;                                
            f.vartype = "undefined";            
            n2 = new Node(t);
            n2.name = n2.value;
            n2.scopeId = scopeId;			                       
        	if(t.match(jsdef.AS) || t.match(jsdef.COLON)) 
        	{
                t.mustMatch(jsdef.IDENTIFIER);
                f.vartype = new Node(t).value;                
                n2.vartype = f.vartype;
        	} 
        ...
        else
        {
        	...
        	f.returntype = f.vartype;        	
        }

        //Set all access modifiers to false so the property body is unaffected
        t.static = false;
        t.private = false;
        t.public = false;
        t.protected = false;
    	...
    	f.body = Script(t, x2);
    	if(f.getter) f.body.returntype = f.vartype;
    	t.mustMatch(jsdef.RIGHT_CURLY);
        f.end = t.token().end;

        f.functionForm = DECLARED_FORM;
        x.funDecls.push(f);
                
        f.value = f.name;
        f.name = (f.getter ? "__get_" : "__set_") + f.name;
        f.body.name = f.name;       
        f.property = true;
    ...
    function FunctionDefinition(t, x, requireName, functionForm) 
    {
        ...
        if ((t.match(jsdef.AS) || t.match(jsdef.COLON)) && t.mustMatch(jsdef.IDENTIFIER)) 
        {        	
        ...
        f.body = (function(t, x) 
        {
        	...
        	do {
        		...
        		if (t.token().type == jsdef.PRIVATE) 
        		{
        			...
        			{
        				n.push(PropertyDefinition(t, x, false, DECLARED_FORM));
					}
