...
#ifndef fxCore_Defines_h
#define fxCore_Defines_h
...
//IDL GLenum Regex: (?:\s*const\s+GLenum\s+(\w+).*(?:\s*\/\*.*\*\/\s*)*)
//IDL WegGL Regen: (?:.*?)(\w+)\(.*?\);
...
#define fxJSIsInstanceOf(O, C) JSValueIsInstanceOfConstructor(fxCRL::js_context, O, C, nullptr)
 ...
 */
#endif /* JAVASCRIPT_ENGINE_V8 */

// TESTING
#ifdef ENABLE_HQ_AUDIO
    ...
    #define AUDIO_FORMAT_MONO AL_FORMAT_MONO8
    #define AUDIO_FORMAT_STEREO AL_FORMAT_STEREO8
#endif
    ...
    #define fxAPIGetChangedTouchesLength(E) ([[(UIEvent*)E allTouches] count])
        ...
        #define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "fxCore", __VA_ARGS__))
        #define LOGW(...) ((void)__android_log_print(ANDROID_LOG_WARN, "fxCore", __VA_ARGS__))
    #else
        #define LOGI(...)
        #define LOGW(...)
    #endif
...
#endif /* defined(fxCore_Defines_h) */
