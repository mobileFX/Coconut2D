...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function __init_narcissus(__global)
{
	__global.narcissus = {};
	var jsdef = __global.jsdef = __global.narcissus.jsdef = {};
	...
	[
		...
		"optional",
		"missing",				/* optional - does not generate default value */
		"super",
	...
	var keywords = jsdef.keywords = (function ()
	{
		...
		for(var i = 0, j = tokens.length; i < j; i++)
		{
			...
			jsdef[a_const] = i >> 0;
			tokens[t] = i;
			__global.narcissus.jsdefNames[i] = a_const;
	...
	var jsdef = __global.narcissus.jsdef;
	...
	var rx_space = /^\s+/;
	var rx_comment = /^\/(?:\*(?:[\S\s])*?\*\/|\/.*)/;
	var rx_jsdoc = /[\s\t]*\x2f\x2a\x2a([\n\N\w\W.]+?)[\s\t]+\x2a\x2f/mg;
	var rx_n = /\n/g;
	var rx_number = /^0[xX][\da-fA-F]+|^0[0-7]*|^\d+/;
	var rx_string = new RegExp("^\\x22(?!\\x22)(?:\\\\.|[^\\x22])*\\x22|^\\x27(?!\\x27)(?:\\\\.|[^\\x27])*\\x27|^([\\x27\\x22]{3})((?:(?!\\1)[\\s\\S])*)\\1|^\\x22\\x22|^\\x27\\x27", "");
	var rx_ident = /^[$\w]+/;
	var rx_lines = /\r?\n/gm;
	var rx_lines2 = /\r?\n\r?/gm;
	var rx_quoted_string = new RegExp("^['\\x22]|['\\x22]$", "gm");
	var rx_new_line = /^[\n\r]/;
	...
	Tokenizer.prototype =
	{
		...
		get: function ()
		{
			...
			for(;;)
			{
				...
				var match = (this.scanNewlines ? rx_tab : rx_space).exec(input);
				if(match)
				{
					var spaces = match[0];
					this.cursor += spaces.length;
					var newlines = spaces.match(rx_n);
					if(newlines)
						this.line_start += newlines.length;
					input = this.input();
				}
				if(!(match = rx_comment.exec(input)))
					break;
				var comment = match[0];
				this.cursor += comment.length;
				newlines = comment.match(rx_n);
				if(newlines)
					this.line_start += newlines.length;

				//JSDoc
				if((match = rx_jsdoc.exec(comment)))
				{
			...
			else if((match = rx_number.exec(input)))
			{
				token.type = jsdef.NUMBER;
				token.value = parseInt(match[0]);
			}
			else if((match = rx_ident.exec(input)))
			{
				var id = match[0];
				token.type = jsdef.keywords(id) || jsdef.IDENTIFIER;
				token.value = id;
			}
			else if((match = rx_string.exec(input)))
			{
				token.type = jsdef.STRING;

				if(match[1])
				{
					this.line_start += match[2].split(rx_lines).length - 1;
					token.value = match[2].split(rx_lines2);
				}
				else
				{
					this.line_start += match[0].split(rx_lines).length - 1;
					token.value = match[0].replace(rx_quoted_string, "");
				}
			...
			else if(this.scanNewlines && (match = rx_new_line.exec(input)))
			{
	...
	function Node(t, type)
	{
		...
		if(type)
			this.nodeType = __global.narcissus.jsdefNames[type];
	...
	function matchVartype(t, node, typeProp, skip)
	{
		...
		if(t.peek()==jsdef.DOT)
		{
			// Module (a.b) or Package (a.b.c.d)

			var names=[];

			for(;t.peek()==jsdef.DOT;)
			{
				t.mustMatch(jsdef.DOT);
				t.mustMatch(jsdef.IDENTIFIER);
				names.push(t.token().value);
			}

			node.vartype_module = names.slice(0, names.length-1).join(".");
			//node[typeProp] = vartype = node.vartype_module + "." + names[names.length-1];
			node[typeProp] = vartype = names[names.length-1];
		}
	...
	function Statement(t, x)
	{
		...
		default:
			...
			t.unget();
			n.expression = Expression(t, x);
			n.nodeType = __global.narcissus.jsdefNames[n.type];
	...
	__global.narcissus.jsparse = jsparse;
