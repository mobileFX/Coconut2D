...
{
	__scene = scene;
	DeviceMessage* touchEvent = engine->getTouchEvent();
	if(touchEvent && touchEvent->type != String("touchend") && touchEvent->type != String("mouseup"))
	{
		touchEvent = nullptr;
	}
	Time parentClipsDuration = parentClip ? parentClip->__childWithMaxTimelineDuration->__timeline->__durationInTime : __timeline->__singleFrameDurationTime;
	if(parentClip)
	{
	...
	if(__image)
	{
		...
		apply(__currentFrame, scene->__modelViewMatrix);
		__currentFrame->scaleX /= __image->__pixelRatioScale;
		__currentFrame->scaleY /= __image->__pixelRatioScale;
